...
| `dept-via-lab`, `dept-via-contact` | SERP + page read (Tier 2B / Tier 2A) | Fire only when the lane produced a department. A blocked lane produces none — and neither does a lane that named Name 1 again, which `_echoes_name1` rejects at the UC 13 write (enrichment/orchestrator.py:8689) rather than at finalise, where the drop left the flag standing over a slot the block authority had already refilled. |
