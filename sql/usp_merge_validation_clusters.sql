CREATE PROCEDURE [Mapping].[usp_MergeValidationClusters]
    @chrEntity SYSNAME,
...
BEGIN
    ...
    ------------------------------------------------------------------
    -- Static SQL: parse the /api/dedup/cluster-block response
    ------------------------------------------------------------------
    ...
    WITH (
        row_id NVARCHAR(100) N'$.row_id',
        block_id NVARCHAR(100) N'$.block_id',
        cluster_id NVARCHAR(100) N'$.cluster_id',  -- content hash string, not INT
        routing NVARCHAR(30) N'$.routing',
        signature_id NVARCHAR(100) N'$.signature_id',
        confidence FLOAT N'$.confidence',
        reasoning NVARCHAR(MAX) N'$.reasoning'
    );

    ------------------------------------------------------------------
    -- Dynamic SQL: only database and schema are spliced in
    ------------------------------------------------------------------
    DECLARE @sql NVARCHAR(MAX) = N'      MERGE ' + @tgt + N' AS tgt      USING #src AS src         ON tgt.Customer = src.row_id        AND tgt.[code] LIKE @pat ESCAPE @esc      WHEN MATCHED THEN UPDATE SET          tgt.[Block ID]     = src.block_id,          tgt.[Cluster ID]   = src.cluster_id,          tgt.[Routing]      = src.routing,          tgt.[Signature ID] = src.signature_id,          tgt.[Confidence]   = src.confidence,          tgt.[Reasoning]    = src.reasoning;';
