...
5. [The Enrichment Pipeline: Stage by Stage](#the-enrichment-pipeline-stage-by-stage)
   ...
   - [Stage 2c: Wikidata crosswalk lane](#stage-2c-wikidata-crosswalk-lane)
     - [The corroboration-only pass](#the-corroboration-only-pass)
   ...
   - [Stage 5b: Page-read corroborator](#stage-5b-page-read-corroborator)
     - [Containment — one name inside another](#containment--one-name-inside-another)
...
**Where it sits.** ROR miss **and** GLEIF miss **and** no registry identifier on the record → **`wikidata_crosswalk`** → the existing [web/domain evidence](#website-domain-department-domain--search-term-resolution) → the existing LLM lanes. The lane has a [second, much smaller role](#the-corroboration-only-pass) on the records this precondition excludes. Concretely: on the company branch it runs immediately after [`_run_lei_lookup`](#stage-2-company-tier-1--gleif--lei-registry-lookup) returns a miss and *before* `company_canonical`; on the research branch it runs where the ROR-miss passthrough would otherwise write the input straight through (GLEIF never runs there, so the record has no registry identity either way).
...
##### The corroboration-only pass

**Entry point:** `Orchestrator._retain_wikidata_website` · **Flag:** `WIKIDATA_DOMAIN_CORROBORATION` (default on) · **Runs in** `_finalise_and_return`, before the website paths propose a candidate

The precondition above declines a record that already holds a registry identifier, and is right to: a register outranks a wiki, and a record with an identity has nothing to gain from a pointer to one. It does have one thing to gain — the item's **`P856`**. That claim is the [domain ownership guard's](#2b--ownership-guard-domain_ownership_guard_enabled-default-on) second condition, and it is the only evidence that can verify a candidate domain the name comparator structurally cannot reach: an acronym host, a contraction of the name, a brand domain. All three are named in that guard's own "known cost".

So on a registry-resolved record the lane runs in a **corroboration-only** mode. The gauntlet above is unchanged — a film, a foreign namesake or a low-scoring label is refused for exactly the same reasons — and the *only* effect of a match is to stash the website claim. It follows no pointer, writes no name, proposes no `operating_name`, and touches no field; the record it hands back differs in exactly one key, which is asserted as a test.

Three further guards, each of them a call **not** made: the crosswalk lane already ran on this record; the record holds no registry identity (that is the crosswalk lane's own population); or the domain already came *from* a registry, in which case a second statement of the same fact buys nothing. On the 99-row S2 evaluation file the pass fired 22 times, returned a `P856` 3 times, and verified 1 domain that had been `domain-unverified`.

> This is the one request this calibration adds, which is why it has its own flag rather than running unconditionally. `WIKIDATA_DOMAIN_CORROBORATION=false` leaves the lane with its crosswalk role alone, and the `+wikidata` domain witness then fires only where the crosswalk lane itself matched. The pure-insert assertion covers both entry points.
...
>
> **No new similarity machinery.** Name comparison reuses `enrichment.registry_match.names_agree`, which is `enrichment.tier1_lei._name_match_score` — `token_sort_ratio`, max of the raw score and the legal-form-stripped score — at `PAGE_NAME_MATCH_THRESHOLD`, which inherits `LEI_NAME_MATCH_THRESHOLD`'s derivation because it is the same comparison, **or** normalised token-set containment. See [Containment](#containment--one-name-inside-another) below; the same function answers the same question for the [cross-source gate](#cross-source-consistency-fix-d), so the two cannot drift.

**Fetch.** The domain root; if it answers, the first of `/impressum`, `/legal`, `/about`, `/contact` that returns 2xx, appended to the root's text rather than replacing it (a name in a footer and an address on `/contact` are both needed). **The footer is fetched explicitly.** `PageFetcher.REMOVE_TAGS` decomposes `<footer>` before body text is taken — correct for the department, contact and profile readers, for which a footer is per-page boilerplate — so the copyright line this step depends on was being deleted before the reader saw it, and the reader was left with hero copy whose only name is the trading brand. `PageContent.footer_text` now carries it as a separate slice (tail-sliced at 600 chars, because the copyright line ends a footer and the link columns start it), and `_readable_text` appends it back marked `[footer]` for this reader alone. Hard timeout `PAGE_READ_TIMEOUT_SECONDS`. Cached on the **domain** in memory *and* on disk under `PAGE_FIXTURE_DIR` — one JSON file per domain, so a batch naming the same organisation twice reads it once and a thesis re-run reproduces its corroboration decisions rather than re-litigating them against today's web. `PAGE_FIXTURE_REPLAY_ONLY` refuses to fetch anything not already recorded.
...
**Extract.** One LLM call, as a constrained *reader*: input is the fetched text only, output is `{stated_org_name, stated_city, stated_region, stated_country, stated_postal_code, legal_form_present}` or nulls. The prompt instructs the model to return null when the page states no organisation identity — a brand logo or a slogan is not a statement — and never to fill a field the page omits from its own knowledge, because a remembered headquarters city would decide the very location test being run. A page that names itself twice — "LabQ" across the top, "Labq Clinical Diagnostics, Inc." in the copyright line — has to be resolved one way or the other, and the rules were silent, so whichever form the reader met first won and that is the short one. **Rule 7** settles it: among the forms the page *actually prints*, the most complete wins. It is not a licence to invent — rule 6 still forbids assembling a name from parts or adding a legal form the page does not show — and it is only reachable because the footer now reaches the reader. Prompt version `page_read/v2`.

**Compare and act.**

| Outcome | Condition | Consequence |
|---|---|---|
| `corroborated` | name consistent AND location consistent-or-neutral | the site named this organisation: `domain-unverified` withdrawn, the candidate **accepted** into `domain` through the single write path (ownership condition 6, provenance `web:{domain}:provisional`), and `operating_name` written with the same provenance; feeds [`unchanged-verified`](#the-three-unchanged-name-1-states) |
| `contradicted` | name consistent, stated location differs | the *verdict* is recorded and counted. Whether it decides anything is a question of **granularity**, and one constant answers it for both directions — see below |
| `name_mismatch` | the page names a different organisation | an **accepted** domain is withdrawn (`domain`/`website_url` cleared, `domain-unverified` raised, a `page_identity` guard rejection logged); an already-unverified one keeps its flag, and nothing is accepted. Either way the reason gains a clause naming what the page said |
| `fetch_unavailable` / `parked` / `no_identity` | we could not look, or the page says nothing | nothing changes; counted. A block can neither accept a domain nor withdraw one |
...
**One granularity floor, both directions.** `ACTIONABLE_LOCATION_SCOPES = {region, country}` was derived for the *withdrawal* rule — a name difference alone withdrew four correct domains on the chemspeed batch, and requiring the page to *also* place the organisation in a different state or country left the one true positive standing and dropped all four false ones. It now governs **acceptance** too, read through one predicate at each site (`location_decides`, `page_identifies_record`). "The page places this organisation somewhere else" is a single claim, and it cannot be strong enough to take a domain back while being too weak to withhold one. So a page that names the organisation and states a different city *inside the record's own state* is a plant-and-head-office difference and accepts; a different state or country does not. The registry comparator reaches the same answer by a different route — it holds a *set* of addresses and downgrades a city difference inside an agreeing region to a note before the verdict — because it can see the region agree and a single page read cannot.

#### Containment — one name inside another

`token_sort_ratio` is length-sensitive **by design**; that property is what makes it safe as GLEIF's verification guard. It also means the commonest shape in a customer master is the shape it scores lowest: the record carries a brand plus a division or a legal form, and the other source carries the brand. A site stating its brand against a record carrying brand-plus-division scores 51.9. That is not a different organisation.

So `names_agree` offers a second way to agree, and only a second way — the ratio is untouched and still primary:

1. normalise both names with the existing `tier1_ror._normalise_for_tokens` (casefold, dashes and punctuation to spaces, legal suffixes canonicalised);
2. drop the tokens that carry no identity — `tier1_lei._LEGAL_FORM_TOKENS ∪ text_utils._GENERIC_COMPANY_WORDS`, composed rather than re-listed;
3. agree when one token set is a **subset** of the other **and** the subset retains the longer name's **leading** token. Direction-agnostic.

> **The leading token is the whole guard.** Bare subset containment is too weak, and the case that shows it is a record reading "⟨Head⟩ ⟨Brand⟩ Sales LLC" against a page for "⟨Brand⟩ Incorporated": `{brand}` *is* a subset of `{head, brand, sales}`, and the two are unrelated companies. An organisation name in these registries is head-initial — brand first, then the division, the product line or the legal form — so the leading distinctive token is the one that says *which* organisation, and a subset that has dropped it has dropped the identity rather than a qualifier. Keeping it also refuses a subsidiary whose name is contained in its parent's, which is the conservative answer. A name left with no distinctive tokens at all agrees with nothing: "contained in everything" is not agreement.

Containment does **not** feed the [registry tier classifier](#cross-source-consistency-fix-d). That asks a stricter question — is this the same string typed twice — and a brand against a brand-plus-division stays a `fuzzy` match whose address check remains armed.
...
`name1_enriched` is the single exception to that, and only because its competing values are not competing facts: every member already carries the same organisation name — that is what grouped them — and they differ only in how the legal form is spelled, punctuated or cased. The **modal** surface form wins. The batch's own majority spelling is evidence; "which legal form is more correct" is not a judgement this pass is entitled to make.

**A tie breaks on closeness to the names the records were supplied with.** A tie means the batch has no majority spelling, so what settles it should still be a fact about the data — and the old tie-break, earliest tier then batch order, was not one. The demo batch shows the cost. The Coastal trio spells one company three ways, no two records enrich to the same form, so all three tie at one vote each, and row order elected `Coastal Diagnostics Inc.` — a string **none of the three records was ever supplied with**, taken from row 15 purely because it came first. Scoring each candidate against every original in the group elects `Coastal Diagnostics, Inc` instead, which row 17 supplied verbatim.

The originals come from the provenance log (`ProvenanceLog.original_value`), where the input passthrough is an ordinary event — a finalised `EnrichmentResult` carries no `name1_original`, and this is the question the log already answers. Similarity is `token_sort_ratio`, the same matcher the rest of the pipeline compares names with, summed over **all** the group's originals rather than the one belonging to the record that produced the candidate: an elected form answers for the whole group. Scoring per record instead would elect `Coastal Diagnostics` — verbatim from row 16's `COASTAL DIAGNOSTICS` — and silently drop the legal form the other two rows carry. Tier and batch order remain below it, so a group whose records carry no originals still elects deterministically, exactly as before.
...
**Confidence → write semantics.** `high` → write. `low` → write. `none` → leave empty. Provenance alone no longer raises a flag: the old "inferred by LLM — verify" reason said *where the URL came from*, not whether it is right, and Fix 8 replaced it with the evidence-based `domain-unverified` — raised when the ownership guard (§2b) can tie the candidate to nothing. The candidate is still written, at `low`; the code is what says how far to trust it. Path B's chosen SERP title rides along on `WebsiteResolution.title` as read-only evidence for the guard's on-domain condition; it never influences selection.
...
2. **An independent witness states this website.** The ROR record or the Wikidata item the record resolved to publishes an official website — ROR's `links[]` of `type == "website"`, Wikidata's `P856` — whose **registrable domain is the candidate's**, compared through the same `canonicalise_domain` every value here goes through, so `https://www.jnj.com/` against a candidate `jnj.com` is an agreement and not a string difference. Two systems that never consulted each other naming the same website is what a *witness* is, so this is the only web-derived accept that reaches `verified` in the provenance column: `web:{domain}:verified+wikidata` or `web:{domain}:verified+registry`, per the actual witness. Ranked directly below registry provenance and above every scored condition — it is a second source, not a better similarity. No call is made for it: see [§2c](#2c--where-the-witness-claims-come-from).
3. **Name similarity.** RapidFuzz `token_sort_ratio` between Name 1 (normalised by `tier1_ror._normalise_for_tokens`) and the domain label (split on hyphens only) reaches **`DOMAIN_NAME_MATCH_THRESHOLD`**. Concatenated words are deliberately *not* segmented — guessing word boundaries inside `aumbiotech` produces false confidence.
4. **Email domain match.** The record carries an email whose registrable domain is not a consumer provider (gmail, googlemail, outlook, hotmail, live, yahoo, aol, icloud, gmx, web.de, t-online.de, protonmail, …). When the candidate could not be verified on its own this **replaces** it: a record holding `ORDERS@MERIDIANLABS.COM` already knows the domain better than a search result does (`meridianlabs.ai`).
5. **On-domain search evidence.** The candidate came from a SERP result *on that domain* whose page title or H1 contains the Name 1 tokens — **all** significant (≥4-char) tokens, not just one. The SERP layer already admits a result on a single overlap, which is exactly how a stranger's "… Biotech …" page slips through.
6. **Page identity.** The page served *by* the candidate domain states this organisation's own name, and does not place it in a different state or country — decided by [Stage 5b](#stage-5b-page-read-corroborator) and applied on a second pass, because the page read runs after the guard has first judged the candidate. Last in precedence and deliberately: a page fetched from the domain it vouches for is **one** source, not two, so it accepts at `provisional` and can never reach `verified` (hard rule 4). It is still the best evidence available about a candidate that reached none of the five above — the site itself, asked directly — and refusing to consult it while flagging the record *"verify this domain"* asked a reviewer to do by hand the one check the pipeline had run and then discarded.

Precedence is registry → witness → name → email → on-domain evidence → page identity; the first hit wins and names the accepting condition. Name similarity is consulted *before* the email so a well-matched candidate is never clobbered by an unrelated address on the record (a distributor's mailbox).

> **Conditions 2 and 6 are additions, not relaxations.** Neither moves a threshold, and each one either finds a second source or asks the site. Candidates that reached none of the six are refused exactly as before: `Owens Corning Sales LLC → corning.com` (no witness, no page — corning.com answers 403 — and `owens`, the distinctive head token, is absent from anything the candidate could show), and a Battle Creek plant record against `battlecreekmich.com`, whose page states an unrelated restaurant group.

**None holds** → the candidate is still written to `domain`, at `web:{domain}:low` (see [shipping the unverified domain](#shipping-the-unverified-domain)), `website_url` is left empty, and the record carries the `domain-unverified` code scoped to the `domain` field. Nothing can mask it: flags are no longer appended as tiers run but computed once, from final state, at the end of `finalise` (see [Flag Rules](#flag-rules)), and a record simply carries every code that applies to it.

Two records both reading "Cardinal Instruments" (Tampa and Boston) fail name similarity against `cardinalhealth.com` and `cardinalguitars.com` alike. Both ship their candidate at `low`, flagged, for a reviewer to accept or clear.

#### Shipping the unverified domain

The guard answers one question — *did anything independently tie this site to this organisation* — and a candidate that satisfies none of the six conditions used to be discarded, leaving an empty `Domain` and a flag naming the site in prose. Checked against the batch, **most of those candidates are the right site**. The guard's failure is usually an absence of corroboration rather than evidence of a wrong answer: `fishersci.com` really is Fisher Scientific's site, and "fishersci" is simply a contraction `token_sort_ratio` cannot reach. Blanking the column made a reviewer go and find a value the pipeline had already found.

So the candidate ships, and the column says what it is worth. `enrichment/orchestrator.py::_ship_unverified_domain` runs at the end of `finalise`, immediately before the flag decision, and writes it with `UNVERIFIED_DOMAIN_RULE` — which `situation_for` maps to a contradicted situation, so the provenance reads **`web:{domain}:low`** and never `provisional`. `domain-unverified` still fires. Nothing is claimed that was not claimed before; the same doubt is recorded in the same two places. What changed is that a reviewer **confirms** a value instead of going and finding one, and the reason prose changed with it — from *"a candidate website (x) was found… confirm x before using it"* to *"the domain shown (x) was found on the web but nothing independently tied it to this organisation — confirm it"*.

Three conditions stop it, all narrowing:

| Condition | Why |
|---|---|
| The guard left no candidate **string** | An older bare-`True` marker names no site; there is nothing to write |
| `domain` is already set | Any later stage's answer is the better one — the [page read](#stage-5b-page-read-corroborator) promoting this same candidate to `provisional` is the common case |
| The page read **refuted** it | `_domain_refuted`, set by `_withdraw_domain`. The one class where the pipeline has evidence *against* the domain rather than merely no evidence *for* it: the site's own page states another organisation's identity. `johnsoncontrols.com` for "AB Controls, Inc." does not come back |

A country-rejected candidate never reaches this path at all — the resolver declines to record one, precisely so it can never be presented as an answer. `website_url` also stays empty: it is the homepage of a domain that was *attributed*, and this one was not. Only the column a reviewer acts on is populated.
...
**Known cost.** Domains that are *contractions* of the name (`fishersci.com` for "Fisher Scientific", `massgeneralbrigham.org` for "Brigham and Women's") and bare acronym hosts (`ufl.edu`) are unreachable by name similarity. They still resolve through registry provenance, an on-domain SERP title, or the record's email. A Path C (LLM) guess with none of those is not *attributed* — but it is no longer dropped: it ships at `low` with `domain-unverified`, which is the case the section above exists for. Hard rule 1 still binds, because `low` is not `verified` and a model's guess never becomes one.

**Telemetry.** Batch summary: `domain_from_registry`, `domain_from_witness`, `domain_from_email`, `domain_from_serp` (name similarity or on-domain evidence), `domain_from_page`, `domain_rejected_unverified`. Each rejection also logs one line naming the candidate and Name 1.

Set `DOMAIN_OWNERSHIP_GUARD_ENABLED=false` to A/B disable the guard; canonicalisation (§2a) still applies.

#### 2c · Where the witness claims come from

Condition 2 consults claims the pipeline **already fetched while resolving the identity**; the witness path adds no request of its own.

| Witness | Claim | Retained by |
|---|---|---|
| `registry` | ROR's `links[]` website for the matched entity | `consistency.record_registry_identity`, at every point a registry match is accepted onto a record — the first-pass ROR match, the [Stage 5](#stage-5-tier-1-re-lookup-after-canonicalisation) retry, and both [crosswalk](#stage-2c-wikidata-crosswalk-lane) lanes |
| `wikidata` | the matched item's `P856` official website | the crosswalk lane at match time, and — on a record the registries already resolved — the lane's [corroboration-only pass](#the-corroboration-only-pass) |

GLEIF publishes no website field, so a GLEIF-only record has no `registry` witness to offer; its witness, when it has one, is Wikidata's.

The claims must survive to *finalisation*, because that is when a candidate domain first exists to compare them against. They ride the record as the transient `_src_stated_websites` and `_wikidata_website` keys and are dropped in `finalise` with the rest of the working state. A claim naming a **different** website is not a witness and is not evidence against the candidate either: the candidate falls through to the conditions it always had.
...
**Preconditions (all must hold, else `None`):** any organisation type; `department_domain` not already set; Name 2 is not a Tier 3 guess that finalise will drop (`_is_droppable_tier3_guess`); Name 2 does not name a legal entity (`_names_a_legal_entity` — a trailing LLC / Inc / Co / Corp / Ltd / GmbH… or a DBA marker); the institution **`domain`** (base) is present; a usable Name 2 that is **not** an admin desk (`is_admin_unit` — §5a, skipped before any fetch/SERP), **not** an address/location fragment, and **not** a granular unit (lab/group/core/facility); and at least one significant token or acronym derives from the cleaned Name 2.
...
0. **Constructed subdomain** — build prefixes (acronym if 2–6 chars; the two longest ≥4-char tokens; plus their 4/3-char prefixes, `chem`←`chemistry`), form `{prefix}.{base}`, drop any that is a generic host (**§5g**), fetch the rest concurrently, first that verifies wins.
...
2b. **On-domain path page** (reuses stage-2 results, no new SERP) — for departments hosted at a *path*. Candidates whose host **is the record's own `domain`** are dropped first (**§5h**), then generic **hosts** (**§5g**), then hosts that name no unit (**§5i** — the path is dropped at the emit point, so it can no more redeem a repository or a catalogue host than it can redeem a newsroom one), then those whose path is non-department content (**§5b** generic-path blocklist: `news, news-events, events, story, article, blog, calendar, archive, colloquium, seminar, …`); the rest are ranked by **canonicality (§5c)** — a shallow landing page beats a deep, dated (`/2020/`), or sub-page (`/undergrad`, `/people`, `/faculty`, …) URL — then verified best-first. Stores the full URL.
3. **Cross-domain SERP** (`DEPT_PROBE_CROSS_DOMAIN`, **default `false` — §6**) — `"{cleaned} {name1}"`; skips third-party hosts, generic hosts (**§5g**) and hosts that name no unit (**§5i** — the top result for "\<department\> \<institution\>" is usually the parent brand's own homepage); first verified host wins (`radiology.hopkinsmedicine.org` for a JHU dept). A second, unrestricted SERP call; enable only for split-domain academic medical centres.
...
**Generic-host guard — `_host_prefix_is_generic` (§5g).** A newsroom or administrative subdomain (`news`, `newsroom`, `events`, `media`, `press`, `blog`, `magazine`, `today`, `alumni`, `giving`, `admissions`, `library`, `jobs`, `login`, …) is never a department home, however well its content verifies. The list also carries the **cross-cutting services** an institution runs for *every* one of its units — `digitalcommons`, `scholarworks`, `repository`, `catalog`, `bulletin`, `courses`, `libguides`, `guides`, `facultyhonors`, `honors`, `profiles`, `research`, `canvas`, `registrar`, `athletics`, … Each publishes one page **per department** (a repository collection, a catalogue entry, a subject guide, an award citation), so the page verifies and the *path* carries the department while the *host* names the service. §5i refuses those on its own; listing them stops the probe a stage earlier, before a verification fetch is spent. `_score_dept_candidate` caps such hosts at 0 for the host-based stages, but stages 0, 2b and 3 accept candidates *without* scoring them — so all four paths share this one guard. Without it a story URL such as `news.mit.edu/2026/chemistry-…` verifies (the page genuinely discusses the department), survives the §5b path check (its path is `/2026/chemistry-…`, not `/news/…`) and — once `canonicalise_host` drops the path — ships as `https://news.mit.edu`. Only a *subdomain* label is judged: the base is stripped when known, otherwise a host counts as a subdomain only when it carries more than two labels, so a registrable domain that happens to be named `press.org` is left alone.
...
> ⚠️ This does mean a stage-2b **path** winner loses its path — and with the path goes the only thing that named the unit. `clas.ufl.edu/chemistry` would emit as `clas.ufl.edu`, which names the college, not the department. Since the column never ships a deep link, the answer is not to emit the college but to emit nothing: **§5i** refuses any host that does not name the unit itself, which is what a path winner's host by definition does not do unless it *also* carries the unit's name. A department hosted only at a path on someone else's host therefore ends as `department_domain = None`.

**Never the organisation's own domain — §5h.** A department page hosted on the institution's *registrable* domain rather than a subdomain (`boston.gov/departments/city-clerk`, `nasa.gov/ames/space-biosciences`, `lbcc.edu/fire`) is the case where that trade-off pays nothing: strip the path and what is left is the **Domain** column verbatim, naming the institution a second time and the unit not at all. Worse, it is not inert — a non-null `department_domain` corroborates Name 2 out of `unverified-inference` (§Flags) and `search_term_2` falls back to a handle derived from the host, so the department column shipped `NASA`, the value `search_term_1` already carried. Every other probe stage already refuses a base host (`_host_of` returns `None` for it); the guard makes stage 2b agree.

Enforced twice: **stage 2b** skips such candidates while collecting, so the probe falls through to stage 3 instead of short-circuiting on a value that cannot survive canonicalisation (and spends no verification fetch on it); and **`finalise`** drops any surviving value whose canonicalised host equals `domain`, whatever route wrote it. The emit-point guard runs **before** `compute_flags` and `derive_search_terms` — the two readers — and catches the §5e case where the resolved probe base is a subdomain (`gc.cuny.edu`) and so never equalled `domain` inside the probe. The comparison is against `domain`, not `base`, for the same reason: a page at `gc.cuny.edu/chemistry` *does* stay distinct from `cuny.edu` and is kept. The outcome for a genuinely on-base department is `department_domain = None` — no web home located — which is the honest answer, not a duplicate.

**The host that ships must name the unit — §5i.** §5h is the most visible case of a wider defect, not a case of its own: what the column says is a **bare host**, so a host that names something other than the unit is wrong however well the page behind it verified. Four probe stages accept a candidate, and only two of them ever tested the host — stages 1 and 2 score it (`_score_dept_candidate` gives 0 unless the subdomain prefix carries a needle), while stages **2b** and **3** accept on evidence that does **not** survive into the column: the unit named in a URL *path*, or anywhere on the page body. Every one of these shipped from a real batch:

| Shipped | Record's Name 2 | What the host actually names |
|---|---|---|
| `digitalcommons.pvamu.edu`, `digitalcommons.usf.edu` | Chemistry and Physics; Chemical & Biomedical Engineering | the institutional repository |
| `catalog.smu.edu` | Biological Sciences | the course catalogue |
| `libguides.csun.edu` | Chemistry and Biochemistry | the library's subject guides |
| `facultyhonors.umich.edu` | Howard Hughes Medical Institute | an award citation |
| `pll.harvard.edu`, `sph.umich.edu`, `artsci.uc.edu` | Chemistry & Biochemistry; Life Sciences; Chemistry | a *different* unit — professional education, public health, the umbrella college |
| `massgeneralbrigham.org`, `idfellowship.massgeneralbrigham.org` | Rheumatology, Immunology and Allergy; Global Health | the parent health system, reached through the §5f redirect from `brighamandwomens.org` — so §5h never saw it |
| `bigfuture.collegeboard.org` | Physics | a third-party college directory |
| `nationwidechildrenshospital.tumblr.com` | The Research Institute at Nationwide Children's | the hospital's blog account on a site builder |

Each of those pages genuinely describes the unit — that is why verification passed — and none of those hosts is the unit. Worse than useless: a non-null `department_domain` corroborates Name 2 out of `unverified-inference` (§Flags) and feeds `search_term_2`, so the wrong host also silences the flag that would have sent the row to a reviewer.

`_host_names_the_unit` states the test the scored stages already applied, once more at the point every route has to pass. A host names the unit when a **label of its own** carries a needle (`_dept_needles` — Name 2's significant tokens plus its acronym, in both the raw `S&M` and letters-only `sm` spellings):

* a label **below its registrable domain** — `chem.ufl.edu`, `heb.fas.harvard.edu`, `chemistry.gc.cuny.edu` (§5e), `rad-onc.medschool.umich.edu` (a compound label is matched by its parts too); or
* the **registrable label itself**, but only when that registrable domain is not the organisation's — a unit with a domain of its own names itself there (`hbs.edu` for Harvard Business School), while a parent brand's homepage does not.

Matching is `_seg_matches_needle` (substring, or a shared ≥3-char leading prefix: `chem`↔`chemistry`), tightened for host labels by `_label_matches_needle`: a **2–3 character needle is an acronym** and names the unit only in an acronym-shaped label (≤6 chars) or at the head of a longer one. Left free it matches by accident — `gh` (Global Health) is inside `bri**gh**am` and `ear` (Eye and Ear) is inside `res**ear**ch`, which is exactly how two of the rows above were accepted.

Applied in three places, for the same reason §5h is applied twice: **stage 2b** and **stage 3** skip an inadmissible host while collecting (no verification fetch spent, and the probe falls through to the next stage rather than short-circuiting), and **`finalise`** re-tests whatever survived — against the *settled* Name 2, which is the value the column ships beside — before `compute_flags` and `derive_search_terms` read it. `_dept_host_is_admissible` is the shared test: not a third-party platform, not a §5g service host, and naming the unit.

Replayed over the 40 department domains the university, hospital and government-lab eval batches produced, **12 survive and 28 are dropped**. Every survivor is a department or college host (`engineering.ucdavis.edu`, `npb.ucdavis.edu`, `mse.mtu.edu`, `csm.rowan.edu`, `me.mit.edu`, `heb.fas.harvard.edu`, …); every drop is the organisation named twice, a service host, a parent brand or a third party. Coverage on this column is *supposed* to fall — the alternative is a column a reviewer cannot trust and a flag that never fires.

> **The one thing §5i gives up.** A department hosted only at a path on a host it shares — `usf.edu/engineering/cbe`, `clas.ufl.edu/chemistry` — now yields `None` rather than the college or the institution. Retaining the path for exactly those winners (`https://clas.ufl.edu/chemistry`) is the only way to ship them, and `search_terms` already parses a path-form `department_domain`; it is not done here because "never a deep link in this column" is what §5h rests on, and reversing it is a schema decision, not a bug fix.
...
```
flagged := any(core field confidence == "low")  OR  any NON-ADVISORY code present
...
`domain-unverified` is here for the same reason reached from the domain side. Since the candidate now [ships in the column](#shipping-the-unverified-domain), and most of them are the right site, a queue entry per row would ask a reviewer to confirm the ordinary case — the failure this whole flag model exists to have fixed. What the row must never do is *claim* the value, and it does not: the provenance reads `web:{domain}:low`, the code is in `flag_codes`, and the reason names the domain.

This makes a row with `flag_for_review` **false** and a populated `flag_reason` a valid, expected state — and after these two, the common one. A consumer building the review queue must read the boolean and must not test `flag_codes` for emptiness. A consumer that *wants* the unverified domains filters on the code or on `web:*:low`; what it cannot do is get them from `flag_for_review`.
...
| `registry-location-mismatch` | A ROR or GLEIF match whose **registered locality contradicts** the record's city/state. The match is *kept* — a company relocating within one country is ordinary — and the reason names both places. **Advisory**: emits its reason but does not set `flag_for_review` — see [the derived review flag](#the-derived-review-flag) | `address` |
...
2. **Compare** them pairwise with `registry_match.names_agree` — `tier1_lei._name_match_score` (`token_sort_ratio`, max of raw and legal-form-stripped) at `LEI_NAME_MATCH_THRESHOLD`, **or** [normalised token-set containment](#containment--one-name-inside-another). No new scorer and no new threshold. The containment half matters here because **the two registers do not answer the same question about a name**: GLEIF returns the *formal legal name* and ROR returns the *brand*. On a length-sensitive ratio a formal name against a brand scores in the 50s, and the gate was reading that as two organisations — deleting a correct `ror_id`, the domain that ROR supplied, and the acronym `search_term_1` was derived from, on a row whose two sources agreed.
3. **On agreement between two registries, keep both.** Two registers, queried independently, naming one organisation is a **double-witness confirmation**, not a null result. Both identifiers stay on the record, the name keeps the provenance its own registry earned (a registry-authored value is already `verified`, so there is no new state to invent and none is invented), and the finding is recorded — `_ev_registry_agreement` on the record, a `source_agreement` line on the consistency trace, and the `registry_agreement_count()` batch counter. It raises **no flag**: an agreement is a finding, not a triage signal. Before this, "the gate did nothing" and "the gate found the two sources agreed" left identical evidence behind, and only one of them is a finding.
4. **On disagreement, keep one and null the other.** A registry outranks the web, always. Between GLEIF and ROR, keep whichever agrees better with the record's **own supplied** Name 1 — not with the enriched name, which by that point *is* one of the two claimants. A losing ROR takes its identifier, and the domain and acronym it supplied, with it.
5. **Flag it** — `source-conflict`, naming both entities. A record that silently dropped one of two identities tells a reviewer nothing. Unchanged, byte for byte, for a pair that genuinely disagrees after normalisation.

**The registry location check.** The [locality comparator](#stage-5b-page-read-corroborator) built for the page read now lives in `enrichment/locality.py` and is applied to ROR, GLEIF and the [Wikidata crosswalk](#stage-2c-wikidata-crosswalk-lane) alike, with its rules unchanged: silence is not evidence, only a *stated* place that differs is a contradiction, and the granularity of the disagreement is part of the answer. A registry match whose registered locality contradicts the record's city/state **keeps the match** and gains `registry-location-mismatch` — flag, do not discard, because same-country relocations are common. Combined with Fix C's short-name rule, a short-name match with a location contradiction becomes a no match outright, because the contradiction is precisely the second signal it needed.

**What "the registered locality" is, and how regions are compared.** A registry publishes more than one address and writes regions in more than one alphabet, and both were reading as disagreements when they are not:

- **Every address the registry publishes, and agreement with any one of them is agreement.** GLEIF states `legalAddress` (where the entity is *incorporated*) and `headquartersAddress` (where it *is*), and for a US company those are routinely two different states — Delaware and everywhere else. `compare_registry_addresses` takes the set: **corroborated** when the record agrees with any of them, **contradicted** only when it agrees with none and disagrees with at least one, **neutral** when all of them were silent. The two addresses are not competing claims about one place; they are two true statements about one entity. Entities whose two blocks are identical contribute one address, not two. **ROR goes through the same function with every location it publishes** — `locations[]` is a list, and a multi-site organisation genuinely carries several; truncating it to `locations[0]` is how a record naming a real site of the organisation gets reported as contradicting "the" registered address, which is the same defect GLEIF's two blocks were handed to the comparator to fix. ROR's flat `city`/`region`/`country` keys still name the *primary* location — they are output fields, and only the comparison reads the set. So the two registries can drift apart neither on the rule nor on how much of what the registry says gets consulted.
- **Region normalisation — `US-TX` and `TX` are one region.** GLEIF writes the ISO 3166-2 subdivision code, the SAP record carries the bare state code, and a US page writes the name out in full. So a region is normalised in two steps before any comparison, both inside `enrichment/locality.normalise_region`: an ISO 3166-2 prefix is stripped (`US-TX` → `TX`, `GB-ENG` → `ENG`), then the remainder is folded through `US_REGION_CODES` so `CA` and `California` compare equal. The strip matches the ISO *shape* only — two letters, a hyphen, a short tail — because cutting at the last hyphen instead would turn `Nord-Pas-de-Calais` into `Calais` and invent a region nobody named. It lives in the comparator rather than at each registry's parse site on purpose: a prefix stripped where the GLEIF response is read normalises that one lane and leaves ROR's `country_subdivision_code` fallback, the page read, and the record's own state field still comparing `us-tx` against `texas`. The prose is rendered separately by `region_label`, which expands the code precisely *because* a bare `DE` on a US batch reads as Germany and means Delaware. Countries get the same treatment through `normalise_country` — ROR's `United States` against the record's `US` is one country written twice, and reporting it as a disagreement produced six false contradictions before the fold existed.
- **A contradiction only flags when the name match was weak, and one function decides it for every lane.** `enrichment/registry_match.location_check_action(verdict, tier)` is the rule; GLEIF, ROR and the Wikidata crosswalk all call it, and no lane carries a copy. The trigger is a conjunction: contradicted **and** below exact tier (`enrichment/registry_match.name_match_tier` — `exact`, `fuzzy`, `short_name`, `crosswalk`). Where the record states the registry's name verbatim, the entity has been identified by its name and a disagreeing address is a fact about its geography, not a doubt about which organisation it is; the match stands unflagged and the observation goes to the consistency trace as `registry_location_unconfirmed`, counted per batch. Anything weaker — a fuzzy score, a collision-prone short name, a crosswalk that followed a pointer — is an identification with no anchor, and there the address disagreeing is exactly the doubt the flag exists to raise. A missing tier is not exact: silence about the strength of a match is not a claim that it was strong. **Both lanes classify against every name their registry publishes** — ROR has always ranked the record against all its `names[]` variants, and GLEIF now ranks it against `legalName` plus `otherNames` and `transliteratedOtherNames`, because a record stating a registered *trading* name verbatim has identified the entity as surely as one stating the legal name, and classifying it `fuzzy` fired the address advisory on a match the name had settled. [Containment](#containment--one-name-inside-another) deliberately does **not** feed this classifier: it answers the looser "do these name one organisation" question, and the tier asks the stricter "is this the same string typed twice". Exactness forgives case, punctuation, a legal form one side **omits** (`Arkema` / `ARKEMA INC.`) and a legal form one side **abbreviates** (`Huntsman Corp` / `HUNTSMAN CORPORATION`) — the last of those because forgiving an omitted suffix while penalising an abbreviated one ranked `Huntsman` above `Huntsman Corp` for naming the same entity, which is backwards, and was the entire reason record 13017466 wore the flag. It forgives nothing else: `Smith Inc` / `Smith LLC` and `Smith Inc` / `Smith Corp` are pairs of distinct legal entities and the register is the authority that says so.

- **Region is asked during selection, not only after it.** A registry that returns two entities of one name has not identified either; the record's region against both registered addresses is what tells them apart, and asking it only after a winner was picked meant the tiebreak happened first and the region was left to complain about the result. It ranks below registration status and above name score — every candidate at that point has already cleared the name gate — and it is inert when the record states no region. It resolves a near-tie the ambiguity margin would otherwise refuse, and never creates one. The country guard reads both addresses for the same reason.

On the 100-row chemspeed batch these two narrowings took `registry-location-mismatch` from 19 rows to 1, with 7 `registry_location_unconfirmed` and 3 `registry_location_note` lines in the trace behind it — see [`determinism_findings.md` §8.1](determinism_findings.md).
...
|---|---|---|
| `operating_name` | `Operating Name` | the organisation name the candidate website **states about itself**, read from the page — the most complete form the page prints, which is usually the footer copyright line rather than the masthead brand |
...
Deterministic golden-record election over the clustered rows (no LLM). Each duplicate cluster elects a winner; losers point at it, and every member gets `election_status = "cluster"`. A row with no cluster (or a cluster of one) is `unique` and self-references. The golden fields are always filled — every `golden_record_id` is a proposal by design, and review happens downstream of this pipeline. Doubtful merges go to the Issues list instead: `low_confidence_merge` when the adjudication confidence is below `CONFIDENCE_MERGE_THRESHOLD` (default `0.95`, env-overridable — a pure data retune that never re-runs the LLM), `all_blocked_cluster`, and `empty_scoring_payload`.
...
> **Phase 3 contract:** Phase 3 reads `is_golden_record` / `golden_record_id` directly — there is no approval state in this pipeline. Clusters worth a second look are the ones named in the `Issues` list.
...
`apply_batch_consensus` converges organisation-level fields across a finalised batch, in place. Groups by address block (`derive_block_id`, reused from `dedup/signatures.py`) then by canonicalised Name 1 plus a compatible legal form. A group with exactly one registry identity propagates `ror_id`, `lei_id`, `name1_enriched`, `domain`, `website_url` and `record_type` from a deterministically elected donor; a group with none falls back to `_consensus_name_form` (modal Name 1 spelling, ties broken by `_input_affinity` — closeness to the Name 1 values the records were supplied with, read back from provenance) plus unanimous-gap-fill on the remaining fields, never choosing between competing values. Inheriting records are marked `source = "batch_consensus"`. `PROPAGATED_FIELDS` and `NEVER_PROPAGATED` are module-level data so the exclusion of department-level fields is readable and testable. Never merges, drops or reorders a record; never touches `tier_used` or any flag — it runs after every record is finalised, so the flags are already settled. Full description in [Stage 6: Batch consensus](#stage-6-batch-consensus).
...
`corroborate()` fetches a candidate domain, has the LLM read what the page *states*, and compares that statement with the record. `fetch_pages` does the root-plus-imprint fetch through `PageCache`; `read_page` runs the constrained reader; `compare` / `compare_location` produce the verdict, with location returning a *scope* (`postal`/`city`/`region`/`country`) because the granularities are not equally strong evidence. Name comparison goes through `registry_match.names_agree` — GLEIF's own `_name_match_score`, or [containment](#containment--one-name-inside-another) — so the page reader and the cross-source gate answer that question the same way. `page_identifies_record()` and `location_decides()` are the two predicates the orchestrator reads to accept and to withdraw a domain, and both are written against one granularity floor (`ACTIONABLE_LOCATION_SCOPES`). The location rules themselves now live in [`enrichment/locality.py`](#enrichmentlocalitypy--the-shared-locality-comparator) so the registries can apply the same ones ([Fix D(2)](#determinism-and-reproducibility)), and region comparison folds `CA` and `California` together through the map that module owns. `operating_name_provenance` stamps the date the page was **fetched**, read out of the cache entry — not the date the run executed. Nothing here writes `name1_enriched`. Full description in [Stage 5b](#stage-5b-page-read-corroborator). Tests `test_page_corroborator.py`.
...
`rank_key()` (score DESC, canonical id ASC), `ambiguity_verdict()` and `REGISTRY_AMBIGUITY_MARGIN`, `is_collision_prone()` and `second_signal()`. Shared by ROR and GLEIF so the two registries cannot drift apart on *which candidate wins* and *whether any does*. Decides nothing on its own — the clients call it.

Also the home of the two rules the calibration made shared. `name_match_tier()` / `is_exact_tier()` classify how strongly a name identified an entity, and `location_check_action(verdict, tier)` is the single implementation of the [location-check trigger](#cross-source-consistency-fix-d) that GLEIF, ROR and the Wikidata crosswalk all call. `names_agree()` — the ratio, or [normalised token-set containment](#containment--one-name-inside-another) — answers "do these two strings name one organisation" for the page reader and the cross-source gate alike; `distinctive_tokens()` is its normalisation and `names_agree_by_containment()` the containment half with its leading-token condition. Tests `test_determinism.py`, `test_calibration.py`.

### `enrichment/consistency.py` — Cross-Source Consistency Gate ([Fix D](#determinism-and-reproducibility))

`apply_cross_source_gate()` and `apply_registry_location_check()`, both called once from `finalise` before `compute_flags`. Collects what each source said the organisation is called — and which official website it stated — through `record_registry_identity`, called at every point a registry match is accepted onto a record. Compares the names with `registry_match.names_agree` (the reused ratio at `LEI_NAME_MATCH_THRESHOLD`, or containment): on disagreement it keeps the higher-priority source and nulls the other's fields, leaving `source-conflict` evidence for the flag; on agreement between two registries it keeps both and records the double witness (`registry_agreement_count()`). The location check reads `registry_match.location_check_action`, the shared trigger. Emits one JSON line per acted-on record on `enrichment.trace.consistency`. Tests `test_determinism.py`, `test_calibration.py`.
...
Pure-arithmetic scoring + election over an editable weights table (`weights.json`), separate from the LLM adjudicator so it can be re-run on retuned weights without paying for adjudication again. `elect_golden_records` scores every row (`score_row`) and elects one winner per cluster; `_award_count` implements **G1 (Bernd's year-priority rule)** — a sales-order count only "adds something" when the row owns its cluster's most-recent year. Every row comes out `cluster` or `unique` with its golden fields filled. `detect_issues` derives the potential-inconsistency list, which is where a doubtful merge surfaces (merge confidence below `CONFIDENCE_MERGE_THRESHOLD`, every member blocked, or the whole cluster scored 0); `weights_version` fingerprints the weights for drift detection. Permissive throughout — a missing/dirty value scores 0 and never fails the batch; the one hard error is a duplicated `row_id` (broken upstream join).
...
The single point where `domain` and `website_url` are decided; no other module writes either field. `canonicalise_domain()` reduces a candidate URL to the registrable domain (reusing `extract_domain()`), `canonicalise_host()` does the same for a department domain but keeps the subdomain, and `resolve_domain()` applies the six ownership conditions — registry provenance, an independent witness's stated website (`stated_website_witness()`), name similarity, email domain, on-domain search evidence, page identity — returning a `DomainDecision` the caller writes or a rejection it flags `domain-unverified`. The decision carries the `witness` that accepted it, which is what separates `web:{domain}:verified+wikidata` from `+registry` downstream. Also exposes `email_domain()` / `is_generic_email_domain()` (consumer-provider blocklist) and `name_similarity()`. See [Website, Domain, Department-Domain & Search-Term Resolution §2](#2--domain--the-single-write-path-utilsdomain_resolverpy).
...
| `CONFIDENCE_MERGE_THRESHOLD` | `0.95` | Election: a merge below this adjudication confidence is reported as a `low_confidence_merge` issue (pure data retune, no LLM re-run) |
...
| `WIKIDATA_DOMAIN_CORROBORATION` | `true` | The lane's [corroboration-only pass](#the-corroboration-only-pass) on records the registries already resolved: retains the item's `P856` as a domain witness and does nothing else. The one request this calibration adds; `false` leaves the lane with its crosswalk role alone |
...
| `CONFIDENCE_MERGE_THRESHOLD` | `0.95` | Election (`/api/dedup/score`): report a below-threshold merge as a `low_confidence_merge` issue; never re-runs the LLM |
...
# Domain witnesses, containment, trigger parity, cross-source normalisation
...
Stage 6: batch consensus — group by address block + canonical name + legal form;
         ...
         no registry identity → modal name form (tie → closest to input)
                                 + unanimous-gap-fill only
...
- **A candidate domain can be verified by an independent witness.** The [ownership guard](#2b--ownership-guard-domain_ownership_guard_enabled-default-on) gains two conditions. **Condition 2:** a ROR `links[]` entry or a Wikidata `P856` claim whose registrable domain *is* the candidate's — two systems that never consulted each other naming one website, so it is the only web-derived accept that reaches `verified`, as `web:{domain}:verified+registry` / `+wikidata`. Both of those provenance states existed in the grammar with no code path producing them. **Condition 6:** the page served *by* the candidate states the record's own organisation — last in precedence and `provisional` by construction, because a page fetched from the domain it vouches for is one source, not two. That closes the open item `corroborator_report.md` recorded: the pipeline was reading the page, deciding the site named the record's organisation, withdrawing the flag, and leaving the field empty.
- **[Containment](#containment--one-name-inside-another) in the name comparator.** `token_sort_ratio` is length-sensitive by design, and the commonest shape in a customer master is the one it scores lowest: the record carries brand-plus-division and the other source carries the brand. One token set being a subset of the other — **and** retaining the longer name's leading token — is now a second way to agree. The leading-token condition is the guard: bare containment would admit a different company whose name is a suffix of the record's. One function (`registry_match.names_agree`), used by the page reader and the cross-source gate alike.
- **A blocked fetch is still not evidence** — confirmed, not changed, and now at the accept site as well as the withdraw site.
- **One location-check trigger for every registry lane.** `registry_match.location_check_action` is the rule; GLEIF, ROR and the Wikidata crosswalk call it and no lane carries a copy. Parity of *evidence* too: ROR now compares **every** location it publishes rather than `locations[0]`, GLEIF classifies the tier against `otherNames` as well as the legal name, and both crosswalk lanes now record their locality verdict — the one route that picks an organisation without ever reading its name was also the one route whose address was never checked.
- **The cross-source gate compares normalised entities, not raw strings.** GLEIF returns the formal legal name and ROR the brand; a formal name against a brand scores in the 50s, and the gate was deleting a correct `ror_id`, the domain it supplied and the acronym `search_term_1` came from, on rows whose two sources agreed. Agreement is now a recorded **double-witness confirmation** — both identifiers kept, no flag, a `source_agreement` trace line and a batch counter. Genuine disagreement is unchanged, byte for byte.
- **New:** `WIKIDATA_DOMAIN_CORROBORATION` (the one added request — see [the corroboration-only pass](#the-corroboration-only-pass)), summary counters `domain_from_witness`, `domain_from_page`, `page_domains_accepted`, `wikidata_corroboration_queried`, `wikidata_corroboration_matched`. Tests `test_calibration.py` (67). **No flag code was added or removed**; the vocabulary stays at 14.

### Determinism, the evidence cache and cross-source consistency
...
- **New `flag_codes` column** — a machine-readable list, because a record can hold several conditions and the single concatenated `flag_reason` string did not scale. `flag_for_review` was true **iff** `flag_codes` was non-empty at the time; it has since become [derived](#the-derived-review-flag) in both directions. `flag_reason` is prose only, and states what is uncertain and what to do rather than which tier ran.
...
- **Two tiers, deliberately unequal.** With one registry identity in the group, the registry-resolved donor's values win outright. With none, the pass never picks between competing values — it fills gaps only where the group is already unanimous, and the sole exception is the name form, whose variants are spellings of a name every member already holds (the modal one wins; a tie goes to the spelling closest to what the records were supplied with). That is what converges the Coastal trio, which no registry resolved. It smuggles nothing past the [ownership guard](#2b--ownership-guard-domain_ownership_guard_enabled-default-on): a filled-in domain already satisfied that guard on a record whose Name 1 equals the receiving record's after canonicalisation, so the guard would reach the same verdict.
...
- **Ownership guard (§2b)** — a candidate is attributed to the organisation only with registry provenance, name similarity at `DOMAIN_NAME_MATCH_THRESHOLD` (82), a non-generic email domain on the record, or an on-domain SERP title naming it. Otherwise the candidate ships at `web:{domain}:low` + flag `domain-unverified` — the guard's verdict is recorded in the provenance and the flag rather than by blanking the column. This is the domain-path counterpart to ROR's country guard and GLEIF's name verification; without it `delta.com` was attached to "Delta Analytical" and `cardinalhealth.com` to "Cardinal Instruments", which reads as successful enrichment.
...
- **Election status** — every row is `cluster` or `unique`, with `is_golden_record` / `golden_record_id` always filled. There is no approval step and no `proposed_golden_id` / `approval_status` column: every golden_record_id is a proposal by design.
- **Doubtful merges are issues, not statuses** — a merge below `CONFIDENCE_MERGE_THRESHOLD` (default `0.95`), an all-blocked cluster, or a zero-signal cluster is reported in the Issues list and never changes the election (never re-runs the LLM).
