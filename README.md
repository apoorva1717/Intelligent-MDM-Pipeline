...
**Generic-host guard — `_host_prefix_is_generic` (§5g).** A newsroom or administrative subdomain (`news`, `newsroom`, `events`, `media`, `press`, `blog`, `magazine`, `today`, `alumni`, `giving`, `admissions`, `library`, `jobs`, `login`, …) is never a department home, however well its content verifies. `_score_dept_candidate` caps such hosts at 0 for the host-based stages, but stages 0, 2b and 3 accept candidates *without* scoring them — so all four paths share this one guard. Without it a story URL such as `news.mit.edu/2026/chemistry-…` verifies (the page genuinely discusses the department), survives the §5b path check (its path is `/2026/chemistry-…`, not `/news/…`) and — once `canonicalise_host` drops the path — ships as `https://news.mit.edu`. Only a *subdomain* label is judged: the base is stripped when known, otherwise a host counts as a subdomain only when it carries more than two labels, so a registrable domain that happens to be named `press.org` is left alone.
