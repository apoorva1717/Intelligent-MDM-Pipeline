...
> ⚠️ **The retry can only fire if a tier actually *writes* a changed `name1_enriched`.** `company_canonical.canonical_preserves_identity` rejects a suggestion that changes a distinctive token — including a corrected typo (`MASSACHUSETTS INSITUTE OF TECHNOLOGY` → `Massachusetts Institute of Technology`) and an abbreviation `expand_abbreviations()` does not know, such as a state (`GA Tech`, `FL State Univ`). For those records the pipeline still discards the right answer, one gate earlier than this fix reaches. Variants the guard accepts (`Universität Stuttgart` → `University of Stuttgart`, `Lockheed Martin Corp` → `Lockheed Martin Corporation`, and since the guard began expanding, `Bio-Rad Lab Inc` → `Bio-Rad Laboratories, Inc.`) do reach the retry and converge.
>
...
- **Known limit** — the retry only fires when a tier actually *writes* a changed `name1_enriched`. `canonical_preserves_identity` still rejects a corrected typo (`MASSACHUSETTS INSITUTE OF TECHNOLOGY`) and an abbreviation outside `expand_abbreviations()`'s map — a state, mostly (`GA Tech`, `FL State Univ`) — so those records still discard the right answer one gate earlier than this fix reaches.
  The guard now expands both sides before comparing, so the *unit-word* half of this limit is gone: `Bio-Rad Lab Inc` → `Bio-Rad Laboratories, Inc.` and `Orange County Public Health Lab` → `... Laboratory` are accepted, where `Lab` previously read as a different word from `Laboratories` because the token comparator disables its prefix rule under four characters.
